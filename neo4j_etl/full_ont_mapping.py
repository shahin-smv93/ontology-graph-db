...
def load_rdf_to_neo4j(rdf_graph, neo4j_uri, username, password):
    """Load RDF graph into Neo4j"""
    try:
        driver = GraphDatabase.driver(neo4j_uri, auth=(username, password))
        with driver.session() as session:
            # Write Turtle to file for debugging
            turtle_data = rdf_graph.serialize(format='turtle')
            with open("full_debug_output.ttl", "w") as f:
                f.write(turtle_data)
            
            logging.info(f"Turtle RDF written to full_debug_output.ttl ({len(turtle_data)} characters)")
...
def full_ont_mapping():
    """Main function for full ontology mapping with sensors and measurements"""
    ...
    try:
        ...
        logging.info("Creating RDF graph from enhanced hierarchy...")
        ...
        logging.info("Full ontology mapping completed successfully!")
        
        # Print summary for verification
        print(f"\nSummary:")
        print(f"- RDF graph contains {len(rdf_graph)} triples")
        print(f"- Turtle file saved as full_debug_output.ttl")
        print(f"- Check Neo4j browser for imported data")
        
    except Exception as e:
        logging.error(f"Error in full ontology mapping: {e}")
        raise
